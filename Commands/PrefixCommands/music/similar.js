module.exports = {
    ...
    run: async(client, message, args) => {
        const response = await similar(client, message, args);
        await message.reply(response)
    }
}
async function similar(client, message, args) {
    ...
    const result = await player.search(`https://www.youtube.com/watch?v=${queue.current.identifier}&list=RD${queue.current.identifier}`, message.member)
